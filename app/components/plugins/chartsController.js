    ...
    .controller('chartsCtrl', [
        ...
        function ($scope,$interval,$window,mg_chart_linked_1,mg_chart_linked_2,mg_confidence_band,mg_currency) {
            ...
            // metrics graphics
                ...
                var c3chart_bar_stacked_waypoint = new Waypoint({
                    element: document.getElementById(c3chart_bar_stacked_id),
                    handler: function() {
                        setTimeout(function () {
                ...
                $($window).on('debouncedresize', c3chart_bar_stacked.resize());

                $scope.$on('$destroy', function () {
                    $($window).off('debouncedresize', c3chart_bar_stacked.resize());
                    c3chart_bar_stacked_waypoint.destroy();
                });
            ...
            if ( $(c3chart_area_stacked_id).length ) {
                ...
                $($window).on('debouncedresize', c3chart_area_stacked.resize());

                $scope.$on('$destroy', function () {
                    $($window).off('debouncedresize', c3chart_area_stacked.resize());
                    c3chart_area_stacked.destroy();
                });
            ...
            if ( $('#'+c3chart_scatter_id).length ) {
                ...
                var c3chart_scatter_waypoint = new Waypoint({
                    element: document.getElementById(c3chart_scatter_id),
                    handler: function() {
                ...
                $($window).on('debouncedresize', c3chart_scatter.resize());

                $scope.$on('$destroy', function () {
                    $($window).off('debouncedresize', c3chart_scatter.resize());
                    c3chart_scatter_waypoint.destroy();
                });
            ...
            if($(c3_server_load_id).length) {
                ...
                var liveUpdate = $interval(function () {
